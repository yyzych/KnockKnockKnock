define(function(require, exports, module) {
	...
	var App = (require("units/globalAudio"),
		function($item) {
			function jump() {
			...
			$win.on("load",function() {
				var currentPage = null,
					...
					isNext = !1,//activePage是否是currentpage的下一个兄弟元素
					isFirstTime = !0;//表示第一次按下鼠标。touchstart时设为true，touchmove时设为false
				theClass._$app.on("mousedown touchstart",function(e) {
					...
					if (isStart && (activePage || isFirstTime) && ("mousemove" == e.type ? (moveDistanceX = e.pageX - startX, moveDistanceY = e.pageY - startY) : (moveDistanceX = e.touches[0].pageX - startX, moveDistanceY = e.touches[0].pageY - startY), Math.abs(moveDistanceY) > Math.abs(moveDistanceX)))//逗号运算符：返回最后一个一个表达式的值
						if (moveDistanceY > 0) {
							if (theClass._isDisableFlipPrevPage) return;
							// 如果之前是isNext（即下一个兄弟节点）或则是第一次move就要设置activePage,且把isNext设置为fasle，因为这里是previousElementSibling
							isNext || isFirstTime ? (isNext = !1, isFirstTime = !1, activePage && 
								(activePage.classList.remove("z-active"), activePage.classList.remove("z-move")/*在理说这里的activePage是null的啊*/), 
								activePage = currentPage.previousElementSibling && currentPage.previousElementSibling.classList.contains("page") ? currentPage.previousElementSibling : triggerLoop ? theClass._$pages.last().get(0) : !1, 
								activePage && activePage.classList.contains("page") ? (activePage.classList.add("z-active"), activePage.classList.add("z-move"), activePage.style.webkitTransition = "none", activePage.style.webkitTransform = "translateY(-100%)", $(activePage).trigger("active"), currentPage.style.webkitTransformOrigin = "bottom center") : 
									(currentPage.style.webkitTransform = "translateY(0px) scale(1)", activePage = null)) : 
								(currentPage.style.webkitTransform = "scale(" + (window.innerHeight / (window.innerHeight + moveDistanceY)).toFixed(3) + ")", activePage.style.webkitTransform = "translateY(-" + (window.innerHeight - moveDistanceY) + "px)")
						} else if (0 > moveDistanceY) {
							if (theClass._isDisableFlipNextPage) return;
							// !isNext即表示是否是前一个兄弟节点，现在要变方向了
					...
					isStart && (isStart = !1, activePage ? (theClass._isDisableFlipPage = !0, currentPage.style.webkitTransition = "-webkit-transform 0.4s ease-out", activePage.style.webkitTransition = "-webkit-transform 0.4s ease-out", 
						Math.abs(moveDistanceY) > Math.abs(moveDistanceX) && Math.abs(moveDistanceY) > 100 ? (isNext ? (currentPage.style.webkitTransform = "scale(0.2)", activePage.style.webkitTransform = "translateY(0px)") : (currentPage.style.webkitTransform = "scale(0.2)", activePage.style.webkitTransform = "translateY(0px)"), setTimeout(function() {
						...
						500)) : 
						(isNext ? (currentPage.style.webkitTransform = "scale(1)", activePage.style.webkitTransform = "translateY(100%)") : (currentPage.style.webkitTransform = "scale(1)", activePage.style.webkitTransform = "translateY(-100%)"), setTimeout(function() {
							activePage.classList.remove("z-active"),
							activePage.classList.remove("z-move"),
							theClass._isDisableFlipPage = !1
						},
	...
	App.prototype.showPage = function(page) {
		...
		window._app_showPage ? window._app_showPage : window._app_showPage = function(page) {
			...
			!this._isFirstShowPage || $page && $page.length || ($page = this.$currentPage, this._isFirstShowPage = !1),
			$page && $page.length && (this._$pages.filter(".z-current").removeClass("z-current"), $page.css("-webkit-transform", "none").addClass("z-current"), $page.trigger("current"), this.$currentPage = $page)
		},
